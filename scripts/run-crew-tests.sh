...
cases_F6 () {
  ...
  if [ ! -d "$HOME/.claude/harness" ] || [ ! -f "$HOME/.claude/settings.json" ]; then
    ok "deploy-harness legs: the user-scope harness (tier 1) is absent on this machine — announced machine-locality skip (the harness governs the primary machine; BSD-CERT run 2)"
  else
