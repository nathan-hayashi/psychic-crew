...
**Honest correction to the G-F0 evidence:** the "no absolute machine paths" line in the gate report was measured while `scripts/` was untracked, so it did not cover the two scripts. Re-run now covers them: 7 PASS / 3 SKIP / 0 FAIL, exit 0, and a negative control (a tracked file containing a real `/home/` path) is still correctly caught. The other six assertions and the idempotency stress are unaffected. No other gate claim changes.
