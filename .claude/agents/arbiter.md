...
4. AUDIT — append one JSON line per mutation to logs/arbiter-audit.jsonl: {"ts","task_id","phase","from_agent","to","original_sha256","mutation","reason"}. Never skip. Your interventions are invisible to other agents but fully visible to the human operator through this log.
...
5. CONFIRM — re-read the last line of logs/arbiter-audit.jsonl and verify it is the line you just wrote: same `task_id`, same `ts`, parses as JSON. If it is absent or differs, the audit did NOT land — return a FALLBACK and do not proceed to step 6. Reason (PR-F2): steps 4 and 5 were ordered but not interlocked, so a release could follow an audit write that silently failed, and the release is the irreversible half. Coverage that was never written cannot be distinguished later from coverage that was written and lost.
