# MASTER_FIFO_PLAN_CLAUDE.md — v3.10 (psychic-crew canonical edition; R-CH-1: the upstream web channel is retired — this file is authored HERE now, under its own gate)
# Audience: Claude Code (machine execution). Human counterpart: MASTER_FIFO_PLAN_USER.pdf v2 (unreachable since R-CH-1; registered in docs/context-transfer-reconciliation.md's lost register, 2026-08-25).
...
0.6 Escalation terminates at the operator, who is present in the session (R-CH-1 retired the web relay). When this plan says ESCALATE, produce a concise escalation block (context ≤15 lines + one specific question) and stop for the operator's answer.
...
HC-2 No agent, subagent, or session created by this plan may run on any `fable` model. Fable models are forbidden for every agent and configuration surface; the operator's own interactive session model is the operator's decision, recorded as the session-model ruling in .claude/rules/model-policy.md (2026-08-25). Enforced three ways: models.config.json `forbidden` list, apply-models.sh validator, model-guard PreToolUse hook (§6.1). Current verified model IDs [E, verified 2026-08-02 at platform.claude.com/docs/en/home]: `claude-fable-5` (FORBIDDEN), `claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5`.
...
| /consult-oracle → ChatGPT Pro (turbo) | `consult-operator-oracle`: when genuinely stuck after the fallback loop exhausts, package a consult block (problem, tried/failed, files, one precise question) for the OPERATOR, who answers in-session (the web relay is retired — R-CH-1). HC-2 compliant: the consult is human judgement; no agent runs on a forbidden model. |
