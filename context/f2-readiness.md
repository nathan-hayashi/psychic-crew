...
1. **C-04 first** — append `.claude/state/` to `.gitignore`. Do this *before* installing `sensitive-guard.sh`, which blocks writes touching `.gitignore`. Installing the guard first can deadlock the correction it depends on.
...
| R2 | `$CLAUDE_PROJECT_DIR` unset at hook time → silent no-op | Verify with one trivial hook before wiring the rest. |
...
| R5 | No PreCompact parachute exists *during* F2 — the bootstrap gap | §0.3's 70% early-gate rule plus per-step PROGRESS.md checkpoints until the hook is live and tested. |
