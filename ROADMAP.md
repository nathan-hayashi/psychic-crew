...
- **`DIRECTORY_GUIDE.md` drift — the entry as written is CLOSED, and a different one is open.** The drift this described is resolved: ruling A1b re-exported the plan under the permanent name, **EX-01 is retired**, and the map now matches the tree in both directions for `scripts/` (9) and `context/` (6), asserted by CR-024 on every run. This entry still cited EX-01 four sessions after it stopped existing. **What is open is not the same thing.** C-26 (2026-08-21) found the map says 12 hook scripts against a tree of 14, and that CR-024 polices `scripts/` and `context/` but has never policed `hooks/` — the one directory holding the enforcement layer. The tree half is now bound to `.claude/settings.json`; the map half needs an **operator re-export**, because `DIRECTORY_GUIDE.md` is the plan's §4.3 byte-pinned payload and writing a corrected number into it locally would fail the seed-identity check the build rests on. The F4 precedent — route around the pin by creating what the map already claims — does not apply to a count.
